...
void semSignal(enum RESOURCE r){
    ...
    if (is_empty(&blockedQueues[r])){
        ...
        gui_log("Next Process (pid = %d) is dequeued from the %s blocked queue.\n", nextProcess->pcb->pid, resource_name(r));
        int found=0;
        int count=0;
...
Process* val=dequeue(&general_blocked_queue);
            ...
            if (val==nextProcess){
                ...
                gui_log("Next Process (pid = %d) is dequeued from the general blocked queue.\n", nextProcess->pcb->pid);
            }
            else
        ...
        gui_log("Next Process (pid = %d) is %s\n", nextProcess->pcb->pid, state_name(nextProcess->pcb->state));
        ...
        if (algo == MLFQ) {
            ...
            gui_log("Next Process (pid = %d) is enqueued in MLFQ Queue 0.\n", nextProcess->pcb->pid);
        } else {
            enqueue(nextProcess, &os_ready_queue);
            printf("Next Process (pid = %d) is enqueued in the general ready queue.\n", nextProcess->pcb->pid);
            gui_log("Next Process (pid = %d) is enqueued in the general ready queue.\n", nextProcess->pcb->pid);
        }
        binSem[r] = 0;
    }
}
