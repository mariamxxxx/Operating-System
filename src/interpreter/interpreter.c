...
void callWriteFile(char* filename, char* content){
    ...
    if (contentValue == NULL) {
        ...
        gui_log("Variable %s not found, skipping writeFile\n", content);
        return;
    }
...
char* callReadFile(char* filename){
    ...
    if (result == NULL) {
        ...
        gui_log("ReadFile: could not open file %s\n", resolved_filename);
        return strdup("");
    }
...
void execute_instruction(Process* process) { 
    ...
    gui_log("[ERROR] Attempted to execute a NULL process.\n");
    return;
    }
    ...
    for (int i = 0; i < count; i++) {
        ...
        printf("  -> Part[%d]: %s\n", i, parts[i]);
        gui_log("[DECODE] Instruction parts (processed right-to-left):\n");
        gui_log("  -> Part[%d]: %s\n", i, parts[i]);
        char* part = parts[i];
        ...
        if (strcmp(part, "000") == 0 ){
            if (i>=1){
                ...
                if (resourceType == 0 || resourceType == 1 || resourceType == 2)  {
                    ...
                    gui_log("Syntax Error: Invalid resource type '%s' for semWait in instruction %s\n", parts[i-1], instruction);
                }
            }
            else {
                printf("Syntax Error: Missing resource type for semWait in instruction %s\n", instruction);
                gui_log("Syntax Error: Missing resource type for semWait in instruction %s\n", instruction);
            }
        }
        if (strcmp(part, "001") == 0 ){
            if (i>=1){
                ...
                if (resourceType == 0 || resourceType == 1 || resourceType == 2)  {
                    ...
                    gui_log("Syntax Error: Invalid resource type '%s' for semSignal in instruction %s\n", parts[i-1], instruction);
                }
            }
            else {
                printf("Syntax Error: Missing resource type for semSignal in instruction %s\n", instruction);
                gui_log("Syntax Error: Missing resource type for semSignal in instruction %s\n", instruction);
            }
        }
        if (strcmp(part, "010") == 0 ){
            if (i>=2){
                ...
                gui_log("Syntax Error: Missing variable name or value for assign in instruction %s\n", instruction);
            }
        }
        if (strcmp(part, "011") == 0 ){
            if (i>=1){
                ...
                gui_log("Syntax Error: Missing data to print for print in instruction %s\n", instruction);
            }
        }
        if (strcmp(part, "100") == 0 ){
            if (i>=2){
                ...
                gui_log("Exec: printFromTo from=%d, to=%d\n", from, to);
                callPrintFromTo(from, to);
            }
            else {
                printf("Syntax Error: Missing range values for printFromTo in instruction %s\n", instruction);
                gui_log("Syntax Error: Missing range values for printFromTo in instruction %s\n", instruction);
            }
        }
        if (strcmp(part, "101") == 0 ){
            if (i>=2){
                ...
                gui_log("Syntax Error: Missing filename or content for writeFile in instruction %s\n", instruction);
            }
        }
        if (strcmp(part, "110") == 0 ){
            if (i>=1){
                ...
                gui_log("Syntax Error: Missing filename for readFile in instruction %s\n", instruction);
            }
        }
        if (strcmp(part, "input") == 0 ){
            printf("Process %d is going to take input, please press enter \n", process->pcb->pid);
            gui_log("Process %d is going to take input, please press enter \n", process->pcb->pid);
...
//one?
    ...
    printf("=================================================\n");
    gui_log("=================================================\n");
    gui_log("[DONE] End of instruction for PID %d\n", process->pcb->pid);
    gui_log("=================================================\n");
}
